...
if has_cmd pyenv; then
  log "Using pyenv to manage Python ${PY_SERIES}.x"
  ...
  if ! pyenv versions --bare | grep -qx "$SELECTED_PY"; then
    log "Installing Python $SELECTED_PY via pyenv"
    export PYTHON_CONFIGURE_OPTS="--enable-shared"
    export CFLAGS="-O2"
    pyenv install "$SELECTED_PY"
  else
    log "Python $SELECTED_PY already in pyenv"
  fi
  log "Setting local pyenv version to $SELECTED_PY"
  ...
  log "Found system python3.13 ($(python3.13 -V))"
  PY_BIN="python3.13"
else
...
if [ -d "$VENV_DIR" ]; then
  log "Virtualenv exists, verifying Python version"
  ...
  if [ -n "${SELECTED_PY:-}" ]; then
    if [ "$CURRENT_FULL" != "$SELECTED_PY" ]; then
      log "Recreating venv for Python $SELECTED_PY (was $CURRENT_FULL)"
      rm -rf "$VENV_DIR"
      info "Creating virtualenv at $VENV_DIR using $PY_BIN"
      "$PY_BIN" -m venv "$VENV_DIR"
    else
      log "Venv matches desired version $SELECTED_PY"
    fi
  else
    if [ "$CURRENT_SERIES" != "$PY_SERIES" ]; then
      log "Recreating venv for series $PY_SERIES (was $CURRENT_SERIES)"
      rm -rf "$VENV_DIR"
      info "Creating virtualenv at $VENV_DIR using $PY_BIN"
      "$PY_BIN" -m venv "$VENV_DIR"
    else
      log "Venv matches desired series $PY_SERIES"
    fi
  fi
else
  info "Creating virtualenv at $VENV_DIR using $PY_BIN"
