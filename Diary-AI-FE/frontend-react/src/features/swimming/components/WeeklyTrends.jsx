...
export default function WeeklyTrendsSwimming({ activities: incomingActivities } = {}) {
  ...
  const sportActivities = useMemo(
    () => (Array.isArray(activities) ? activities : []).filter(a => isSportMatch(a, 'swimming')),
  ...
  const rows = useMemo(() => (weeklyGroupsFilled || weeklyGroups).map((w, i, arr) => {
    ...
    return {
      ...
      avgSwolf: w.avgSwolf ?? null,
      avgStrokeRate: w.avgStrokeRate ?? null,
    };
  ...
  const columns = [
    ...
    { key: 'avgSwolf', label: 'SWOLF', format: v => v!=null ? Number(v).toFixed(0) : '—' },
    { key: 'avgStrokeRate', label: 'Strokes/min', format: v => v!=null ? Number(v).toFixed(1) : '—' },
  ...
  return <WeeklyTrends title="Swimming Weekly" rows={rows} columns={columns} defaultSortKey="week" defaultSortDir="desc" />;
}
