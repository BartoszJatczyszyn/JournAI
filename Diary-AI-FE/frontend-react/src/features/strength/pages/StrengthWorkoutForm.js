...
import { listMuscleGroups, searchExercises, createWorkout, getSuggestion, listTemplates, upsertTemplate, listWorkouts, getWorkout } from 'features/strength/api';
...
const ExerciseBlock = ({ ex, idx, onUpdate }) => {
  const [suggestion, setSuggestion] = useState(null);
  useEffect(() => {
    let active = true;
    const run = async () => {
      try {
        const s = await getSuggestion(ex.exerciseDefinitionId);
...
export default function StrengthWorkoutForm() {
  ...
  // Prefill activityId from query param if provided
  useEffect(() => {
    try {
  ...
  // If templateId provided via URL, auto-load it once templates arrive
  useEffect(() => {
    if (!initialTemplateId || templates.length === 0) return;
    const tpl = templates.find(t => String(t.id) === String(initialTemplateId));
    if (tpl) {
      setName(tpl.name || '');
      setNotes(tpl.notes || '');
      setExercises((tpl.exercises || []).map((e, idx) => ({ ...e, order: e.order || (idx + 1) })));
      setInitialTemplateId(''); // ensure it runs once
    }
  ...
  const loadLastWorkout = async () => {
    try {
      const list = await listWorkouts({ limit: 1 });
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0,1fr))', gap: 12 }}>
        ...
        <div>
          <label>Garmin Activity ID (strength_training)<br/>
            <input value={activityId} onChange={e => setActivityId(e.target.value)} placeholder="e.g. 1234567890" />
          </label>
        </div>
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        {exercises.map((ex, i) => (
          <div key={i}>
            <ExerciseBlock ex={ex} idx={i} onUpdate={(patch) => updateExercise(i, patch)} />
