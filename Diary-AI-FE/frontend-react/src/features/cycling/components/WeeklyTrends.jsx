...
export default function WeeklyTrendsCycling({ activities: incomingActivities } = {}) {
  ...
  const sportActivities = useMemo(
    () => (Array.isArray(activities) ? activities : []).filter(a => isSportMatch(a, 'cycling')),
  ...
  const rows = useMemo(() => (weeklyGroupsFilled || weeklyGroups).map((w, i, arr) => {
    ...
    return {
      ...
      avgSpeedKmh: w.avgSpeedKmh ?? null,
      avgPowerW: w.avgPowerW ?? null,
      avgCadenceRpm: w.avgCadenceRpm ?? null,
  ...
  const columns = [
    ...
    { key: 'avgSpeedKmh', label: 'Speed (km/h)', format: v => v!=null ? Number(v).toFixed(1) : '—' },
    { key: 'avgPowerW', label: 'Power (W)', format: v => v!=null ? Number(v).toFixed(0) : '—' },
    { key: 'avgCadenceRpm', label: 'Cadence (rpm)', format: v => v!=null ? Number(v).toFixed(0) : '—' },
  ...
  return <WeeklyTrends title="Cycling Weekly" rows={rows} columns={columns} defaultSortKey="week" defaultSortDir="desc" />;
}
