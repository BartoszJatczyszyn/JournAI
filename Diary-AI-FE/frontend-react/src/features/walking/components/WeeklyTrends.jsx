...
export default function WeeklyTrendsWalking({ activities: incomingActivities } = {}) {
  ...
  const sportActivities = useMemo(
    () => (Array.isArray(activities) ? activities : []).filter(a => isSportMatch(a, 'walking')),
  ...
  const columns = [
    ...
    { key: 'steps', label: 'Steps', format: v => typeof v==='number'? v.toLocaleString():'-', tooltip: (v,r,prev)=> (typeof v==='number' && prev)?`This week: ${v.toLocaleString()} steps; Last: ${typeof prev.steps==='number'?prev.steps.toLocaleString():'—'}`:null },
  ...
  return <WeeklyTrends title="Walking Weekly" rows={rows} columns={columns} defaultSortKey="week" defaultSortDir="desc" />;
}
