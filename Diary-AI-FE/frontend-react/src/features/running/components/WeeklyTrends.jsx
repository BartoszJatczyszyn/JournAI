...
export default function WeeklyTrendsRunning({ activities: incomingActivities } = {}) {
  // Use activities passed from page; if not provided, render empty state (pages now pass sportActivities)
  const activities = useMemo(
    () => (Array.isArray(incomingActivities) ? incomingActivities : []),
    [incomingActivities]
  );
  const { isSportMatch } = require('shared/lib/activityUtils');
  const sportActivities = useMemo(
    () => (Array.isArray(activities) ? activities : []).filter(a => isSportMatch(a, 'running')),
  ...
  // Ensure dashboard daily rows cover the same date span as our sport activities,
  // so the steps fallback can fill all visible weeks (not just ~30 days).
  React.useEffect(() => {
    try {
      if (!Array.isArray(sportActivities) || sportActivities.length === 0) return;
      const times = sportActivities
        .map(a => (a && a.start_time ? new Date(a.start_time) : null))
      ...
      // Inspect current dashboard coverage window
      const rows = (dashboardData?.windowData) || [];
      const rTimes = rows
        .map(r => r?._dayObj?.getTime?.())
        .filter(t => Number.isFinite(t));
      const hasCoverage = (() => {
        if (rTimes.length === 0) return false;
        const rMin = Math.min(...rTimes);
        const rMax = Math.max(...rTimes);
        // If our activity span fits inside current window bounds with a small tolerance, we're good
        // Tolerance: 2 days to avoid off-by-one due to timezones.
        const tol = 2 * msPerDay;
        return (rMin <= (minTs + tol)) && (rMax >= (maxTs - tol)) && (rows.length >= Math.min(spanDays, rows.length));
      })();

      // If coverage is insufficient, request a bigger window (capped to 400 days to stay sane)
      if (!hasCoverage) {
        const reqDays = Math.min(400, Math.max(30, spanDays + 7)); // pad by a week
        fetchDashboardForDays?.(reqDays);
      }
    } catch (e) {
      // silent guard
    }
  }, [sportActivities, dashboardData, fetchDashboardForDays]);

  // Fallback: If steps are missing in weeklyGroups (activities often lack steps),
  // derive weekly steps from dashboard daily rows (windowData/healthData).
  const weeklyGroupsFilled = useMemo(() => {
    try {
      const rowsRaw = (dashboardData?.windowData) || (dashboardData?.healthData?.all) || [];
      // Restrict fallback rows strictly to the visible activity date span, so steps align with other columns
  ...
  // Enrich rows to mirror Activity Weekly table
  const rows = useMemo(() => {
    return (weeklyGroupsFilled || weeklyGroups).map((w, i, arr) => {
      ...
      // Derive steps for running when missing using cadence or step length
      ...
      return {
        ...
        distanceChange: pct, // for sorting via columns
        distance: dist,
        ...
        avgStepsPerMin: w.avgStepsPerMin ?? null,
        avgGctMs: w.avgGctMs ?? null,
        avgVerticalRatio: w.avgVerticalRatio ?? null,
        avgStepLengthM: w.avgStepLengthM ?? null,
      };
    });
  }, [weeklyGroupsFilled, weeklyGroups]);

  const columns = [
    { key: 'trend', label: 'Trend', format: (v, _r) => <span className={v==='↑' ? 'text-green-600' : v==='↓' ? 'text-red-600' : 'text-gray-500'}>{v || '·'}</span>, tooltip: () => 'Direction of change vs previous week' },
    ...
    { key: 'trainingLoad', label: 'Training Load', format: (v, r) => (typeof v === 'number' ? (
      <span className={r.trainingLoadSource === 'derived' ? 'text-indigo-600 dark:text-indigo-400' : ''} title={r.trainingLoadSource === 'derived' ? 'Derived from training_effect × duration (approximate)' : 'Explicit training load value'}>
        {Number(v).toFixed(0)}{r.trainingLoadSource === 'derived' ? '*' : ''}
      </span>
    ) : '-'), tooltip: (v,r,prev)=> (v!=null && prev && prev.trainingLoad!=null)?`This week: ${Number(v).toFixed(0)}; Last: ${Number(prev.trainingLoad).toFixed(0)}`:null },
    ...
    { key: 'avgStepsPerMin', label: 'Steps/min', format: v => v != null ? Number(v).toFixed(1) : '—' },
    { key: 'avgGctMs', label: 'GCT (ms)', format: v => v != null ? Number(v).toFixed(0) : '—' },
    { key: 'avgVerticalRatio', label: 'Vertical Ratio', format: v => v != null ? Number(v).toFixed(1) : '—' },
    { key: 'avgStepLengthM', label: 'Step Len (m)', format: v => v != null ? Number(v).toFixed(2) : '—' },
    { key: 'activeDaysCount', label: 'Consistency', format: v => (
      <span className={
        v>=7 ? 'text-green-600 font-semibold' : v>=5 ? 'text-indigo-600' : v>=3 ? 'text-yellow-600' : 'text-red-600'
      }>{v} / 7</span>
    ), tooltip: (v)=> v!=null?`Active days this week: ${v} / 7`:null },
    { key: 'mini', label: 'Mini Dist', format: (_v, r) => (
      <div className="w-32">
        <Sparkline
          data={(r.dailyDistanceSeries || []).map(v => ({ value: v }))}
          height={26}
          stroke="#0ea5e9"
          fill="rgba(14,165,233,0.15)"
          tooltipFormatter={(pt, di) => `Day ${di+1}: ${Number(pt.value||0).toFixed(2)} km`}
        />
      </div>
    ), tooltip: () => 'Daily distances this week' },
  ];

  return <WeeklyTrends title="Running Weekly" rows={rows} columns={columns} defaultSortKey="week" defaultSortDir="desc" />;
}
