  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Feature facade may return a mapped model directly or the infra layer
        // may return { sleep: { ... } } — handle both shapes defensively.
        const sleepObj = res && res.sleep ? res.sleep : res || null;
        setSleep(sleepObj);
      } catch (e) {
