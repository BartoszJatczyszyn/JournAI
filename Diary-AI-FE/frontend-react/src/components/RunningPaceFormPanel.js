...
export default function RunningPaceFormPanel({ data }) {
  ...
  const rows = [
    { k: 'All Mean Pace', v: data.pace_mean_all, fmt: v => v != null ? `${formatPaceMinPerKm(v)} min/km` : '—', tip: 'Average pace (min/km) for entire period.' },
    { k: 'All Std Dev', v: data.pace_std_all, fmt: v => v != null ? `${formatPaceMinPerKm(v)}` : '—', tip: 'Pace standard deviation (minutes → mm:ss).' },
    { k: 'Recent 7d', v: data.recent_mean_pace_7d, fmt: v => v != null ? `${formatPaceMinPerKm(v)} min/km` : '—', tip: 'Average pace last 7 days.' },
    { k: 'Baseline 30d', v: data.baseline_mean_pace_30d, fmt: v => v != null ? `${formatPaceMinPerKm(v)} min/km` : '—', tip: '30-day baseline average pace.' },
  ...
  return (
    <div className="card h-full">
      <div className="card-header flex items-center justify-between"><h3 className="card-title">Pace Form</h3>
        <span className="text-[10px] text-gray-500">Pace Z-score</span></div>
