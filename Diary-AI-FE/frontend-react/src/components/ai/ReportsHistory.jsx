  ...
  return (
    <div className="card" style={{ marginTop: 24 }}>
      ...
      <div className="card-body">
        <div className="form-row flex flex-wrap gap-3">
          <label>
            Language:
          ...
          <label>
            Limit:
            <input className="input-glass ml-2 w-80" type="number" min={1} max={50} value={limit} onChange={e => setLimit(parseInt(e.target.value || 5))} />
          </label>
          <button className="btn btn-primary" onClick={fetchHistory} disabled={loading}>{loading ? 'Loading…' : 'Refresh'}</button>
        </div>
            ...
            <div className="grid grid-3 gap-3">
              <div>
                ...
                <ul style={{ listStyle: 'none', padding: 0 }}>
                  {history.map((r) => (
                    <li key={r.id}>
                      <button className="btn btn-outline w-full text-left" onClick={() => setSelectedA(r)}>
              ...
              <div>
                ...
                <ul style={{ listStyle: 'none', padding: 0 }}>
                  {history.map((r) => (
                    <li key={r.id}>
                      <button className="btn btn-outline w-full text-left" onClick={() => setSelectedB(r)}>
                        {r.day} ({r.language}) [{r.days_window}d]
                      </button>
                    </li>
                  ))}
