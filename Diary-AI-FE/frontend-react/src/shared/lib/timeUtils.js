...
/**
 * Format a duration value as MM:SS or H:MM:SS when >= 60 minutes.
 * Accepts values in minutes or seconds (heuristic detection via durationToMinutes).
 * Returns null for invalid input.
 */
export const formatDuration = (v, unit = 'auto') => {
  if (v == null) return null;
  // Allow callers to explicitly pass unit: 'seconds' | 'minutes' | 'auto'
  let mins = null;
  if (unit === 'seconds') {
    const n = Number(v);
    ...
    // auto-detect via helper (handles objects and various formats)
    mins = durationToMinutes(v);
  }
  if (mins == null || Number.isNaN(mins)) return null;
  // If mins is large (>=60) format H:MM:SS, else MM:SS
  ...
  if (absSec >= 3600) {
    ...
    return `${sign}${String(hours)}:${String(minutes).padStart(2,'0')}:${String(seconds).padStart(2,'0')}`;
  }
