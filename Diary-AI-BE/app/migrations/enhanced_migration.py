...
try:
    from app.utils import DbConfig, load_env  # when imported via AI/run_migration.py
...
class EnhancedGarminMigrator:
    ...
    def apply_schema_changes(self):
        ...
        with self.engine.begin() as conn:
            ...
            # Ensure helpful indexes for common analytics queries
            try:
                conn.exec_driver_sql("CREATE INDEX IF NOT EXISTS idx_gds_day ON garmin_daily_summaries(day);")
                conn.exec_driver_sql("CREATE INDEX IF NOT EXISTS idx_gss_day ON garmin_sleep_sessions(day);")
            except Exception as e:
                logger.warning(f"Could not ensure daily/sleep day indexes: {e}")
            try:
                # Minute-level tables (if present)
                conn.exec_driver_sql("CREATE INDEX IF NOT EXISTS idx_gha_day ON garmin_heart_rate_data(day);")
            except Exception:
                pass
            try:
                conn.exec_driver_sql("CREATE INDEX IF NOT EXISTS idx_gsd_day ON garmin_stress_data(day);")
            except Exception:
                pass
            try:
                # Composite index used in running analytics and other sport-specific queries
                conn.exec_driver_sql(
                    "CREATE INDEX IF NOT EXISTS idx_ga_sport_day ON garmin_activities(LOWER(sport), COALESCE(day, start_time::date));"
                )
                logger.info("Ensured analytics indexes (sport/day composites and day indexes)")
            except Exception as e:
                logger.warning(f"Could not ensure garmin_activities composite index: {e}")
