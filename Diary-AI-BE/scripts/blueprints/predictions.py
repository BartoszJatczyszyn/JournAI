...
@router.get("/energy")
def predict_energy(days_ahead: int = Query(7, ge=1, le=60)):
    try:
...
@router.get("/sleep")
def predict_sleep(days_ahead: int = Query(7, ge=1, le=60)):
    try:
...
@router.get("/mood")
def predict_mood(days_ahead: int = Query(7, ge=1, le=60)):
    try:
...
def predict_comprehensive(days_ahead: int = Query(7, ge=1, le=60)):
    try:
