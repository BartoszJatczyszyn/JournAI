...
# Concurrency limiter and timeout for heavy analytics
_ANALYTICS_CONCURRENCY = int(os.getenv("ANALYTICS_CONCURRENCY", "4"))
_ANALYTICS_TIMEOUT = float(os.getenv("ANALYTICS_TIMEOUT", "60"))
_SEM_ANALYTICS = asyncio.Semaphore(_ANALYTICS_CONCURRENCY)

async def _limited(fn, *args, timeout: float | None = None):
    async with _SEM_ANALYTICS:
        try:
            return await asyncio.wait_for(asyncio.to_thread(fn, *args), timeout or _ANALYTICS_TIMEOUT)
        except asyncio.TimeoutError:
            from fastapi import HTTPException
            raise HTTPException(status_code=504, detail="analytics processing timeout")
...
async def enhanced_comprehensive(days: int = Query(90, ge=1, le=365)):
    try:
        insights = await _limited(_enhanced.get_comprehensive_insights, days)
        return {
            "status": "success",
...
async def enhanced_correlations(days: int = Query(90, ge=1, le=365)):
    try:
        ...
        correlations = await _limited(_enhanced.calculate_advanced_correlations, data)
        return {
            "status": "success",
...
async def enhanced_clusters(
    ...
    try:
        ...
        cluster_data = await _limited(_enhanced.perform_cluster_analysis, data, clusters)
        return {
            "status": "success",
...
async def enhanced_temporal_patterns(days: int = Query(90, ge=1, le=365)):
    try:
        ...
        patterns = await _limited(_enhanced.analyze_temporal_patterns, data)
        return {
            "status": "success",
...
async def enhanced_recovery(
    ...
    try:
        if start_date and end_date:
            data = await _limited(_enhanced.get_comprehensive_health_data_range, start_date, end_date)
            trend_series = await _limited(_enhanced.get_recovery_trend_range, start_date, end_date)
        else:
            data = _enhanced.get_comprehensive_health_data_v2(days)
            trend_series = await _limited(_enhanced.get_recovery_trend, days)
        recovery = {} if not data else _enhanced.analyze_recovery_patterns(data)
        try:
            pattern_details = await _limited(_recovery_patterns.analyze_recovery_patterns, days)
...
async def stress_comprehensive(days: int = Query(30, ge=1, le=365)):
    try:
        analysis = await asyncio.to_thread(_stress.analyze_stress_patterns, days)
        return {
            "status": "success",
...
async def activity_comprehensive(days: int = Query(30, ge=1, le=365)):
    try:
        analysis = await asyncio.to_thread(_activity.analyze_activity_patterns, days)
        return {
            "status": "success",
...
async def debug_running_samples(days: int = Query(90, ge=1, le=365)):
    ...
    try:
        ...
        """ % days
        total_res = await async_execute_query(q_total, fetch_one=True) or {}
        ...
        """ % days
        pace_res = await async_execute_query(q_pace, fetch_one=True) or {}
        ...
        WHERE (LOWER(sport) = 'running' OR LOWER(sport) = 'run')
          ...
          AND start_time::date >= CURRENT_DATE - INTERVAL '%s days'
        """ % days
        vo2_res = await async_execute_query(q_vo2, fetch_one=True) or {}
        ...
        """ % days
        labels = await async_execute_query(q_labels) or []
