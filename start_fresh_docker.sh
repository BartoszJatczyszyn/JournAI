...
docker compose run --rm -T --entrypoint "" backend python run_migration.py || {
  echo "Migration failed. Check logs or run: docker compose run --rm --entrypoint \"\" backend python run_migration.py" >&2
  exit 1
}
